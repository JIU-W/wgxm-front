  ...
  <div>
    ...
    <div class="table">
      <el-table :data="tableData" strip @selection-change="handleSelectionChange">
        ...
        <el-table-column prop="userId" label="序号" width="80" align="center" sortable></el-table-column>
  ...
  methods: {
    ...
    save() {   // 保存按钮触发的逻辑  它会触发新增或者更新
      this.$refs.formRef.validate((valid) => {
        if (valid) {
          this.$request({
            url: this.form.userId ? '/userInfo/update' : '/userInfo/save',
    ...
    del(id) { //单个删除
      this.$confirm('您确定删除吗？', '确认删除', { type: "warning" }).then(response => {
        this.$request.delete('/userInfo/delete/' + id).then(res => {
    ...
    delBatch() {   // 批量删除
      ...
      this.$confirm('您确定批量删除这些数据吗？', '确认删除', { type: "warning" }).then(response => {
        this.$request.delete('/userInfo/delete/batch', { data: this.ids }).then(res => {
          if (res.code === '200') {   // 表示操作成功
            this.$message.success('操作成功')
            this.load(1)
          } else {
    ...
    load(pageNum) {  // 分页查询
      ...
      this.$request.get('/userInfo/selectPage', {
        params: {
    ...
    //处理pageSize(每页的条数)的变化，实现对pageSize变化的监听（分页组件会把改变后的pageSize传过来）
